...
public class PolygonDrawer extends NodeDrawer {
    ...
    /**
     * Creates a swing drawable polygon object by adding a Polygon shape to the
     * list of shapes of the drawable and correctly setting its position.
     *
     * @param component
     *            tikz polygon to draw
     * @param panel
     *            panel to draw onto
    ...
    /**
     * Creates a awt Polygon from the sides and length of the given TikzPolygon.
     * No responsability on the position.
     *
     * @param polygon
     *            the polygon tikz object to create an awt polygon from.
    ...
    private Polygon getAwtPolygon(TikzPolygon polygon) {
        ...
        return new Polygon(getPolygonXCoords(vertices, (int) Converter.centimetersToPixels(size)),
                getPolygonYCoords(vertices, (int) Converter.centimetersToPixels(size)), vertices);
    }
    ...
    /**
     * Set the position of the given polygon. uses swing coordinates. The
     * polygon is centered and translated by the node coordinates converted to
     * swing coordinates
     *
     * @param shape
     *            polygon to position. Type is Shape in order to override parent
     *            method.
     * @param node
    ...
    /**
     * Determines the anchors of the given polygon. Constructs the Polygon shape
     * as if we wanted to make a Drawable. Then, uses the obtained shape to get
     * the vertices and compute the center of each following pair of vertices
     * for anchors.
     *
     * @param node
     *            node to get the anchors from
     * @param panel
     *            panel on which the node is drawn
    ...
    public List<Point2D.Float> getAnchors(TikzNode node, JComponent panel) {
        ...
        int next_i;
        Point2D.Float firstVertex;
        Point2D.Float secondVertex;
        for (int i = 0; i < vertices; ++i) {
            next_i = (i + 1) % vertices;
