...
public class ProjectsDAO {
    ...
    public void create(Project project) throws Exception {
        ...
        try {
            ...
            preparedStatement = initializationPreparedRequest(connection, ProjectRequests.SQL_INSERT_PROJECT, true, project.getUid(),
                    project.getUserID(), project.getPath(), project.getLast_modification(), project.isWrite_default() ? 1 : 0,
                    project.isRead_default() ? 1 : 0, project.getName());
    ...
    public Project findByUid(String uid) throws SQLException {
        ...
        try {
            resultSet = executeQuery(daoFactorySingleton, connection, preparedStatement, resultSet,
                    ProjectRequests.SQL_SELECT_PROJECT_BY_UID, uid);
            if (resultSet.next()) {
    ...
    public ArrayList<Project> getAllReadableProject(int userID) throws SQLException {
        ...
        try {
            resultSet = executeQuery(daoFactorySingleton, connection, preparedStatement, resultSet,
                    ProjectRequests.SQL_PROJECT_GETALLREADABLES, userID);
            while (resultSet.next()) {
    ...
    public void update(Project dbProject) {
        executeUpdate(daoFactorySingleton, ProjectRequests.SQL_UPDATE_PROJECT, dbProject.getPath(), dbProject.getLast_modification(),
                dbProject.isWrite_default(), dbProject.isRead_default(), dbProject.getName(), dbProject.getUid());
    }
}
