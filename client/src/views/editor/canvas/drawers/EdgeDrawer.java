...
public abstract class EdgeDrawer extends ComponentDrawer {
    ...
    /**
     * Creates the drawable swing object for the given edge. By default it is
     * considered undirected.
     *
     * @param component
     *            the edge to create a drawable from
     * @param panel
     *            the panl on which the edge needs to be drawn
    ...
    public DrawableTikzEdge toDrawable(TikzComponent component, JComponent panel) {
        ...
        drawableComponent.addShape(new Line2D.Float(fromPosition(edge, panel), toPosition(edge, panel)));
        return drawableComponent;
    }

    /**
     * Given the edge, determine its first (from) point. The determined point is
     * a swing point. If the edge has no from or to node, set the position as
     * the center of the panel. Else, get the closest from node anchor to the to
     * node.
     *
     * @param edge
     *            the edge to determine the from position
     * @param panel
     *            the panel on which the edge is drawn
    ...
    /**
     * Given the edge, determine its second (to) point. The determined point is
     * a swing point. If the edge has no from or to node, set the position as
     * the center of the panel. Else, get the closest to node anchor to the from
     * node.
     *
     * @param edge
     *            the edge to determine the to position
     * @param panel
     *            the panel on which the edge is drawn
