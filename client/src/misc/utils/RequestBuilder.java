...
public class RequestBuilder {
    ...
    public static Invocation get(final String path) {
        return getClient().path(path).request().header("Authorization", "Bearer " + token).buildGet();
    }
    ...
    public static Invocation post(final String path, final Entity entity) {
        return getClient().path(path).request().header("Authorization", "Bearer " + token).buildPost(entity);
    }

    public static Invocation put(final String path, final InputStream stream, final String type) {
        return getClient().path(path).request().header("Authorization", "Bearer " + token).buildPut(Entity.entity(stream, type));
    }

    private static WebTarget getClient() {
        return ClientBuilder.newClient().target(Network.HOST.COMPLETE_HOSTNAME).register(MultiPartFeature.class);
    }
