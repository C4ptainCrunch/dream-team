...
public class ConflictResolver {
     ...
     *
     * @param localProject
     *            the localProject to resolve conflicts
     * @param serverProject
     *            the serverProject to resolve conflicts
     */
    ...
    /**
     * Resolve conflicts between two projects. One from the user and the
     * server's one.
     *
     * @param userChoice
     *            The conflict resolution choice of the user. It can be PUSH,
     *            PUSH FORCE, USER MERGE, OTHER USER MERGE AND OTHER USER PUSH
     *            FORCE (see projectconflicts.properties).
    ...
    private void resolveLocalDiagrams(String localDiagram, Set<String> serverDiagramNames, String userChoice,
            List<String> resolvedDiagrams) {
    ...
    /**
     * Resolves conflicts between the local and server diffs given the choice of
     * the user The choice can either be: - saveUserVersion : Keeps
     * modifications of the local diffs and server diffs but rejects diffs from
     * server diffs that causes conflicts conflicts from server diffs -
     * saveServerVersion : Keeps modifications of the server diffs and local
     * diffs but rejects diffs from local diffs that causes conflicts
     *
     * @param userChoice
     *            The choice of the user whether the user wants to keep local or
     *            server version or wants to merge versions
    ...
    /**
     * Adds conflict indexes to the resulting diff that will be needed to create
     * the new graph
     *
     * @param diffs
     *            The diffs list from where to get conflict diffs
     * @param conflictIndexes
     *            List of indexes of conflict diffs
     * @param resolvedDiffs
    ...
    private Boolean checkListConflicts(Set<String> diagramNamesList, Set<String> otherDiagramList, List<String> checkedDiagram)
     ...
     *
     * @param diff1
     *            The first diff list to compare
     * @param diff2
     *            The second diff list to compare
     * @return The diffs list containing the differences between the two diffs
     *         lists
     */
    ...
    /**
     * Initialiazes two lists of indexes, one for the local diffs, one for the
     * server diffs. Those indexes indicates which diffs (for local and server
     * diffs) contain conflict elements (ie. same reference node) if the
     * checkOnly is at False. If the checkOnly option is at True, this method
     * stops at the first conflict occurence encountered and return list of size
     * one.
     *
     * @param local
     *            diffs list of the local version
     * @param server
     *            diffs list of the server version
     * @param checkOnly
     *            boolean option to stop at first conflict encountered or not
        ...
        for (Diff localDiff : local) {
            ...
            for (String localReference : localReferences) {
                if (!checkedReferences.contains(localReference)) {
                    Boolean hasConflict = checkIfReferenceIsConflict(server, localReference, conflictsReferences, serverConflictsIndexes,
                            localConflictsIndexes, i, checkedReferences, checkOnly);
    ...
    /**
     * Checks whether the reference found in the local version is found in the
     * server version
     *
     * @param server
     *            diffs list of the server
     * @param localReference
     *            the reference found in the lcal version
     * @param conflictsReferences
     *            Hashset containing the references that cause conflict
     * @param serverConflictsIndexes
     *            List containing the indexes of the diffs of the server version
     *            that causes conflict
     * @param localConflictsIndexes
     *            List containing the indexes of the diffs of the local version
     *            that causes conflict
     * @param i
     *            index of the diffs of the local version
     * @param checkedReferences
     *            Hashset of references already checked -> no need to check
     *            again
     * @param checkOnly
     *            Boolean option to stop at first conflict encountered or not
     * @return Whether the given reference from local diff causes conflicts with
     *         server diffs
     */
    private Boolean checkIfReferenceIsConflict(List<Diff> server, String localReference, HashSet<String> conflictsReferences,
            List<Integer> serverConflictsIndexes, List<Integer> localConflictsIndexes, int i, HashSet<String> checkedReferences,
            Boolean checkOnly) {
        int j = 0;
